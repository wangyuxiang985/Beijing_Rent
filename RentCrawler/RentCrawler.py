...
import datetime
import os
import read_rentdata_excel
import read_rentdata_html
...
class RentMain(object):
    ...
    def run(self):
        try:
            ...
            # 写入html
            read_rentdata_html.create_html(self.config.result_file, conn)
            # 写入excel
            read_rentdata_excel.create_excel(self.config.result_excel_file, conn)
        except Exception, e:
            print "Error:", e.message
        finally:
            # conn.commit()
            # conn.close()
            print "Search Finish,Please open rentdata_result.html to view result"
